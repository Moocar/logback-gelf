...
public class AppenderExecutor<E> {
    ...
    private final GelfConverter<E> gelfConverter;
    private final Zipper zipper;
    private final int chunkThreshold;

    public AppenderExecutor(Transport transport, PayloadChunker payloadChunker,
	    GelfConverter<E> gelfConverter, Zipper zipper, int chunkThreshold) {
	...
	if (payload.length < chunkThreshold) {
	    ...
	    // If the message is too long, then slice it up and send multiple
	    // packets
