 ...
 * 
 * A Payload is the raw Gzipped Gelf JSON message. A Chunk is comprised of a
 * Gelf Chunk header as well as a portion of the payload
 */
public class PayloadChunker {
    ...
    public PayloadChunker(int payloadThreshold, int maxChunks,
	    MessageIdProvider messageIdProvider, ChunkFactory chunkFactory) {
     ...
     * 
     * @param messageId
     *            The unique messageId that all the produced chunks will use
     * @param subPayloads
     *            The raw sub payload data.
	...
	for (byte[] subPayload : subPayloads) {
	    ...
	    chunks.add(chunkFactory.create(messageId, seqNum++,
		    (byte) (subPayloads.size()), subPayload));
	}
     ...
     * @param payload
     *            The full payload
     * @return A list of subPayloads which when added together, make up the full
     *         payload
     */
    ...
    /**
     * Extracts a chunk of bytes from another byte array based on a
	...
	return Arrays.copyOfRange(payload, subPaylod * payloadThreshold,
		(subPaylod + 1) * payloadThreshold);
    }
