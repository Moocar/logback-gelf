...
public class GelfAppender<E> extends AppenderBase<E> {
	...
	try {
	    ...
	    PayloadChunker payloadChunker = new PayloadChunker(chunkThreshold,
		    maxChunks, new MessageIdProvider(messageIdLength,
			    MessageDigest.getInstance("MD5"), hostname),
		    new ChunkFactory(chunkedGelfId, padSeq));

	    GelfConverter<E> converter = new GelfConverter<E>(facility,
		    useLoggerName, useThreadName, additionalFields,
		    shortMessageLength, hostname);

	    appenderExecutor = new AppenderExecutor<E>(transport,
		    payloadChunker, converter, new Zipper(), chunkThreshold);

	} catch (Exception e) {
	    throw new RuntimeException(
		    "Error initialising appender appenderExecutor", e);
	}
    }
	...
	if (splitted.length != 2) {
	    ...
	    throw new IllegalArgumentException(
		    "additionalField must be of the format key:value, where key is the MDC "
			    + "key, and value is the GELF field name. But found '"
			    + keyValue + "' instead.");
	}
